...
class TestCollapsedMapping:
    ...
    def test_first_occurrence_preserved(self):
        """The first action with a colliding task must NOT appear in the override."""
        window, tasks = self._colliding_window()
        override, _ = _propose_repair(window, tasks, dict(ACTION_TO_TASK))
        # refresh_repo_health is first — it keeps build_portfolio_dashboard.
        assert "refresh_repo_health" not in override
...
class TestNoDuplicateReplacements:
    ...
    def test_replacement_tasks_differ_from_original(self):
        """Replacement tasks must not be the same as the original colliding task."""
        window = ["regenerate_missing_artifact", "rerun_failed_task"]
        tasks = ["build_portfolio_dashboard", "build_portfolio_dashboard"]
        override, _ = _propose_repair(window, tasks, dict(ACTION_TO_TASK))
        for action, replacement in override.items():
            assert replacement != "build_portfolio_dashboard", (
                f"{action!r} was 'repaired' to the same task it collided on"
            )
